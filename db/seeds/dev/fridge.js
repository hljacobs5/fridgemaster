  ...
  return knex('recipe_steps')
    ...
    .then(() => {
      const recipePromises = mockRecipes.map(async recipe => {
        ...
        await Promise.all(recipe.steps.map((step, index) => createSteps(knex, step, recipeId[0], index)))
...
function createSteps(knex, step_text, recipe_id, step_num) {
  step_num += 1
  return knex('recipe_steps').insert({step_num, step_text, recipe_id}, 'id')
}
